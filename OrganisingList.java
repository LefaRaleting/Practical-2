...
abstract class OrganisingList {
    ...
    public static int sumRec(ListNode node) {
        ...
        //Your code goes here
        if(node.next==null || node==null){//last key
            return (node.key != null ? node.key.intValue() : 0);
        }
        else{
            return sumRec(node.next) + (node.key != null ? node.key.intValue() : 0);
        }
    ...
    public static int dataRec(ListNode node) {
        ...
        //Your code goes here
        if(node.next==null || node==null ){//last key
            node.data=(node.key != null ? node.key.intValue() : 0);
            return  node.data;
        }
        else{
            node.data=(node.key != null ? node.key.intValue() : 0)*sumRec(node)-dataRec(node.next);
            return node.data;
        }
    ...
    public void delete(Integer key) {
        ...
        if(contains(key)) {
            ListNode trans = root;
            ListNode prev = trans;
            while (trans != null && trans.key != key) {
                prev = trans;
                trans = trans.next;
            }
